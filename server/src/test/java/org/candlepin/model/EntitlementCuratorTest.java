...
public class EntitlementCuratorTest extends DatabaseTestFixture {
    ...
    public void markModifyingDirtyInClause() {
        ...
        when(mockEntityManager.createNativeQuery(anyString())).thenReturn(mockQuery);
        when(mockQuery.setParameter(anyString(), org.mockito.Matchers.anyObject())).thenReturn(mockQuery);
        when(mockQuery.executeUpdate()).thenReturn(entitlementCurator.IN_OPERATOR_BLOCK_SIZE).thenReturn(0);
        entitlementCurator.markModifyingEntsDirtyForTesting(mockEntityManager, ents);
        ...
        when(mockEntityManager.createNativeQuery(anyString())).thenReturn(mockQuery);
        when(mockQuery.setParameter(anyString(), org.mockito.Matchers.anyObject())).thenReturn(mockQuery);
        when(mockQuery.executeUpdate()).thenReturn(entitlementCurator.IN_OPERATOR_BLOCK_SIZE)
            .thenReturn(entitlementCurator.IN_OPERATOR_BLOCK_SIZE)
            .thenReturn(1);
        entitlementCurator.markModifyingEntsDirtyForTesting(mockEntityManager, ents);
        verify(mockQuery, times(3)).executeUpdate();

        // now test with two entitlement blocks
        for (int i = 0; i < entitlementCurator.IN_OPERATOR_BLOCK_SIZE * 2; i++) {
            ents.add(new Entitlement());
        }
        mockQuery = mock(Query.class);
        when(mockEntityManager.createNativeQuery(anyString())).thenReturn(mockQuery);
    ...
    public void listModifyingBatch() {
        ...
        // The ent we just created should *not* be returned as modifying itself:
        int modifiedEnts = entitlementCurator.markModifyingEntsDirty(Arrays.asList(ent, ent1));
        assertEquals(6, modifiedEnts);
    }
    ...
    /**
     * Output of markModifyingEntsDirty shouldn't return any entitlement present in the input even if
     * it modifies some input entitlements
     */
    @Test
